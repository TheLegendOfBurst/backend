...
// Middleware de redirecionamento 301
app.use((req, res, next) => {
  const redirections = {
    '/old-path': '/new-path',
    // Adicione mais redirecionamentos aqui se necessário
  };

  const newPath = redirections[req.path];
  if (newPath) {
    return res.redirect(301, newPath); // Redireciona com o status 301 Moved Permanently
  }

  next(); // Chama o próximo middleware se não houver redirecionamento
});
